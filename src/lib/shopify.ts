...
function parseOrder(raw: Record<string, unknown>): ShopifyOrder {
  const lineItems = raw.lineItems as { edges: { node: ShopifyOrderLineItem }[] }
  return {
...
export async function getCustomerUnfulfilledOrders(shopifyCustomerId: string): Promise<ShopifyOrder[]> {
  const data = await shopifyQuery<{
    customer: {
